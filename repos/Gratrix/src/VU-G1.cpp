...
struct GtxWidget : ModuleWidget
{
	GtxWidget(GtxModule *module) : ModuleWidget(module)
	{
		...
		for (std::size_t i=0; i<GTX__N; ++i)
		{
			addInput(createInputGTX<PortInMed>(Vec(px(0, i), py(2, i)), module, GtxModule::imap(GtxModule::IN1_INPUT, i)));
		}

		addInput(createInputGTX<PortInMed>(Vec(gx(0), gy(2)), module, GtxModule::imap(GtxModule::IN1_INPUT, GTX__N)));

		for (std::size_t i=0, k=0; i<GTX__N; ++i)
		{
			for (std::size_t j=0; j<GtxModule::NUM_LIGHTS; ++j, ++k)
			{
				switch (j)
				{
...
Model *model = Model::create<GtxModule, GtxWidget>("Gratrix", "VU-G1", "VU-G1", VISUAL_TAG);
