...
struct Display : TransparentWidget
{
	...
	Display(GtxModule *module_, const Rect &box_)
	:
		module(module_)
	{
		box  = box_;
...
struct GtxWidget : ModuleWidget
{
	GtxWidget(GtxModule *module) : ModuleWidget(module)
	{
		...
		setPanel(SVG::load(assetPlugin(plugin, "res/Seq-G2.svg")));
		...
		addParam(createParamGTX<KnobFreeSml>                       (Vec(portX[0], portY[0]), module, GtxModule::CLOCK_PARAM, -2.0f, 6.0f, 2.0f));
		addParam(ParamWidget::create<LEDButton>                    (but(portX[1], portY[0]), module, GtxModule::RUN_PARAM, 0.0f, 1.0f, 0.0f));
...

		{
			...
			for (std::size_t row = 0; row < LCD_ROWS; ++row, ++j)
			{
				if (OUT_LEFT ) addOutput(createOutputGTX<PortOutSml>(Vec(gridXl, gridY[j]), module, GtxModule::lcd_val_map(row, 0)));
				if (OUT_RIGHT) addOutput(createOutputGTX<PortOutSml>(Vec(gridXr, gridY[j]), module, GtxModule::lcd_val_map(row, 1)));
			}
			#endif
			...
			for (std::size_t row = 0; row < NOB_ROWS; ++row, ++j)
			{
				if (OUT_LEFT ) addOutput(createOutputGTX<PortOutSml>(Vec(gridXl, gridY[j]), module, GtxModule::nob_val_map(row, 0)));
				if (OUT_RIGHT) addOutput(createOutputGTX<PortOutSml>(Vec(gridXr, gridY[j]), module, GtxModule::nob_val_map(row, 1)));
			}
			#endif
			...
			for (std::size_t row = 0; row < BUT_ROWS; ++row, ++j)
			{
				if (OUT_LEFT ) addOutput(createOutputGTX<PortOutSml>(Vec(gridXl, gridY[j]), module, GtxModule::but_val_map(row, 0)));
				if (OUT_RIGHT) addOutput(createOutputGTX<PortOutSml>(Vec(gridXr, gridY[j]), module, GtxModule::but_val_map(row, 1)));
			}
			#endif
		}

		{
			...
			#if PRG_ROWS
			{
				addParam(createParamGTX<KnobSnapSml>(Vec(g_lcdX[ 4], gridY[j]), module, GtxModule::PRG_ROW_PARAM,    0.0f, LCD_ROWS - 1, 0.0f));
				addParam(createParamGTX<KnobSnapSml>(Vec(g_lcdX[ 5], gridY[j]), module, GtxModule::PRG_COL_PARAM,    0.0f, LCD_COLS - 1, 0.0f));
				addParam(createParamGTX<KnobSnapSml>(Vec(g_lcdX[ 6], gridY[j]), module, GtxModule::PRG_SPAN_PARAM,   1.0f, LCD_COLS    , 1.0f));
				addParam(createParamGTX<KnobSnapSml>(Vec(g_lcdX[ 7], gridY[j]), module, GtxModule::PRG_STRIDE_PARAM, 1.0f, LCD_COLS - 1, 1.0f));
				addParam(createParamGTX<KnobSnapSml>(Vec(g_lcdX[ 8], gridY[j]), module, GtxModule::PRG_NOTE_PARAM,   0.0f,        11.0f, 0.0f));
				addParam(createParamGTX<KnobSnapSml>(Vec(g_lcdX[ 9], gridY[j]), module, GtxModule::PRG_OCTAVE_PARAM, 0.0f,         8.0f, 4.0f));
				addParam(createParamGTX<KnobFreeSml>(Vec(g_lcdX[10], gridY[j]), module, GtxModule::PRG_VALUE_PARAM,  0.0f,        10.0f, 0.0f));
				addParam(createParamGTX<KnobSnapSml>(Vec(g_lcdX[11], gridY[j]), module, GtxModule::PRG_GATE_PARAM,   0.0f,         3.0f, 0.0f));
				++j;
			}
			#endif
			...
			for (std::size_t row = 0; row < NOB_ROWS; ++row, ++j)
			{
				...
				{
					if (GtxModule::is_nob_snap(row))
					{
						addParam(createParamGTX<KnobSnapSml>(Vec(g_nobX[col], gridY[j]), module, GtxModule::nob_map(row, col), 0.0f, 12.0f, 0.0f));
					}
					else
					{
						addParam(createParamGTX<KnobFreeSml>(Vec(g_nobX[col], gridY[j]), module, GtxModule::nob_map(row, col), 0.0f, 10.0f, 0.0f));
					}
				}
			}
			#endif
			...
			for (std::size_t row = 0; row < BUT_ROWS; ++row, ++j)
			{
				for (std::size_t col = 0; col < BUT_COLS; ++col)
				{
					addParam(ParamWidget::create<LEDButton>                           (but(g_butX[col], gridY[j]), module, GtxModule::but_map(row, col), 0.0f, 1.0f, 0.0f));
					addChild(ModuleLightWidget::create<MediumLight<RedGreenBlueLight>>(l_m(g_butX[col], gridY[j]), module, GtxModule::led_map(row, col, 0)));
				}
			}
			#endif
		}
		...
		{
			addInput(createInputGTX<PortInMed>(Vec(px(0, i), py(2, i)), module, GtxModule::imap(GtxModule::GATE_INPUT, i)));
			addInput(createInputGTX<PortInMed>(Vec(px(1, i), py(2, i)), module, GtxModule::imap(GtxModule::VOCT_INPUT, i)));

			addOutput(createOutputGTX<PortOutMed>(Vec(px(8, i), py(2, i)), module, GtxModule::omap(GtxModule::GATE_OUTPUT, i)));
			addOutput(createOutputGTX<PortOutMed>(Vec(px(7, i), py(2, i)), module, GtxModule::omap(GtxModule::VOCT_OUTPUT, i)));
		}

		addInput(createInputGTX<PortInMed>(Vec(gx(0), gy(2)), module, GtxModule::imap(GtxModule::GATE_INPUT, GTX__N)));
		addInput(createInputGTX<PortInMed>(Vec(gx(1), gy(2)), module, GtxModule::imap(GtxModule::VOCT_INPUT, GTX__N)));
	}
};


Model *model = Model::create<GtxModule, GtxWidget>("Gratrix", "Seq-G2-alpha1", "Seq-G2 (alpha)", SEQUENCER_TAG);
